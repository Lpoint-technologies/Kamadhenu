...
print(f"SMS_API_KEY loaded: {'Yes' if SMS_API_KEY else 'No'}")
...
def get_base_url():
    ...
    # Default fallback
    return "https://kamadhenu-2.onrender.com"
...
def get_geofence_polygon(cow_id):
    """Fetch polygon coordinates from DB and return as Shapely Polygon"""
    conn = get_db()
    cursor = conn.cursor()
    
    cursor.execute("SELECT coordinates FROM geofence WHERE cow_id = %s", (cow_id,))
...
def send_telegram_alert(cow_id, lat, lon, cow_name=None):
    ...
    message = (
        f"⚠️ ALERT: Cow{cow_name_text} is outside safe zone!\n"
        f"Cow ID: {cow_id}\n"
        f"Location: {lat}, {lon}\n"
        f"Map: {gmap_link}\n"
...
def get_gps_from_esp32():
    """Fetch GPS data from ESP32 device"""
    try:
        ...
        if response.status_code == 200:
            ...
            return {
                ...
                'lng': float(gps_data.get('lon')),
                'status': gps_data.get('status', 'ok')
            }
    except Exception as e:
        print(f"GPS fetch error: {e}")
    return None

def gps_monitor_for_cow(cow_id, cow_name=None):
    """Background thread to monitor GPS for a specific cow"""
    global tracking_active
    
    print(f"🟢 Started GPS monitoring for cow: {cow_id}")
    
    while tracking_active:
        try:
            gps_data = get_gps_from_esp32()
            
            if gps_data and gps_data['lat'] and gps_data['lng']:
                lat = gps_data['lat']
                lng = gps_data['lng']
                ...
                if polygon:
                    ...
                    if not polygon.contains(point):
                        print(f"⚠️ Cow {cow_id} is OUTSIDE safe zone!")
                        send_telegram_alert(cow_id, lat, lng, cow_name)
                    else:
                        print(f"✅ Cow {cow_id} is INSIDE safe zone")
                        
                # Save GPS location to database
                save_gps_location(cow_id, lat, lng)
...
def start_gps_tracking(cow_id, cow_name=None):
    """Start GPS tracking for a specific cow"""
    ...
    tracking_thread = threading.Thread(
        target=gps_monitor_for_cow,
        args=(cow_id, cow_name),
        daemon=True
    )
...
def get_latest_gps(cow_id):
    """Get latest GPS location for a cow"""
    conn = get_db()
    cursor = conn.cursor()
...
# ================= GPS TRACKING ROUTES =================
'''
@app.route('/track_cow/<cow_id>')
def track_cow(cow_id):
    """GPS tracking page for a specific cow"""
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT cow_id, breed FROM cows WHERE cow_id=%s AND farmer_id=%s", 
...
def api_gps_location(cow_id):
    ...
    # Check if cow belongs to farmer
    cursor.execute("SELECT cow_id, breed FROM cows WHERE cow_id=%s AND farmer_id=%s", 
                  (cow_id, session["farmer_id"]))
    cow = cursor.fetchone()
...
@app.route('/api/check_geofence/<cow_id>')
def api_check_geofence(cow_id):
    """Check if cow is inside geofence based on latest GPS"""
    if "farmer_id" not in session:
        return jsonify({"error": "Unauthorized"}), 401
    ...
    if not gps_data:
        return jsonify({"status": "unknown", "message": "No GPS data"})
    
    polygon = get_geofence_polygon(cow_id)
    
    if not polygon:
        return jsonify({"status": "no_fence", "message": "No geofence set"})
    
    point = Point(gps_data['longitude'], gps_data['latitude'])
...
@app.route('/api/start_tracking/<cow_id>', methods=['POST'])
def api_start_tracking(cow_id):
    """API endpoint to start GPS tracking for a cow"""
    if "farmer_id" not in session:
        return jsonify({"error": "Unauthorized"}), 401
    ...
    start_gps_tracking(cow_id, cow['breed'])
    
    return jsonify({
        "success": True,
        "message": f"GPS tracking started for cow {cow_id}"
    })
    '''
...
def admin_farmers():
    ...
    print("=" * 50)
    print("🔍 Admin Farmers Page Accessed")
    ...
    print(f"📊 Farmers table exists: {table_exists}")
    
    if not table_exists:
        print("❌ Farmers table does not exist!")
        conn.close()
        return render_template("admin_farmers.html", farmers=[])
    
    # Count farmers
    cursor.execute("SELECT COUNT(*) as count FROM farmers")
    count = cursor.fetchone()['count']
    print(f"📊 Total farmers in DB: {count}")
    ...
    for farmer in farmers:
        print(f"   - {farmer['name']} ({farmer['email']}) - ID: {farmer['farmer_id']}")
...
def add_cow():
    ...
    if request.method == "POST":
        ...
        # Generate QR Code
        try:
            qr_data = f"https://kamadhenu-2.onrender.com/cow/{cow_id}"
...
def purchase_cow(sale_id):
    ...
    try:
        ...
        qr_data = f"https://kamadhenu-2.onrender.com/cow/{new_cow_id}"
        qr_img = qrcode.make(qr_data)
...
def confirm_appointment(vet_id):
    ...
    if request.method == 'POST':
        ...
        # ============ DEBUG CODE - ADD THIS RIGHT HERE ============
        print("=" * 60)
        print("🔍 DEBUG - Sending SMS to Vet")
        print(f"   Vet Name: {vet['name'] if vet else 'None'}")
        print(f"   Raw Phone from DB: '{vet['phone'] if vet else 'None'}'")
        print(f"   Phone Type: {type(vet['phone']) if vet else 'None'}")
        # ============ DEBUG CODE END ============
...
def save_fence():
    try:
        ...
        # ========== START GPS TRACKING AFTER SAVING FENCE ==========
        # Get cow name for better alerts
        ...
        return jsonify({
            "status": "success", 
            "message": f"Geofence saved for Cow {cow_id}",
        })
...
def admin_milk_production():
    ...
    # Simplified query - get ALL milk-yielding cows even if no milk records
    query = """
        SELECT 
            c.farmer_id,
            f.name as farmer_name,
    ...
    where_clauses = ["c.cattle_type IN ('cow', 'buffalo')"]
    params = []
    
    if date_from:
        where_clauses.append("(my.date >= %s OR my.date IS NULL)")
        params.append(date_from)
    if date_to:
        where_clauses.append("(my.date <= %s OR my.date IS NULL)")
        params.append(date_to)
    if farmer_id:
        where_clauses.append("c.farmer_id = %s")
    ...
    query += """
        GROUP BY c.farmer_id, f.name, c.cow_id, c.breed, c.milk_yield
    ...
    milk_data_list = []
    for row in milk_data:
        milk_data_list.append(dict(row))
    
    # Get farmers list for dropdown
    cursor.execute("SELECT farmer_id, name FROM farmers ORDER BY name")
    farmers_list = cursor.fetchall()
    farmers_list_data = [dict(f) for f in farmers_list]
    
    # Today's total
    today = dt_date.today().isoformat()
    cursor.execute("SELECT COALESCE(SUM(total), 0) as total FROM milk_yield WHERE date = %s", (today,))
    total_milk_today = cursor.fetchone()["total"] or 0
    ...
    cursor.execute("""
        SELECT 
            ...
            COALESCE(SUM(daily_total), 0) as weekly_total
        FROM (
            SELECT date, SUM(total) as daily_total
            FROM milk_yield
            WHERE date >= %s
    ...
    weekly_avg = weekly_stats["avg_daily"] if weekly_stats else 0
    weekly_total = weekly_stats["weekly_total"] if weekly_stats else 0
    
    # Overall avg milk yield per cow
    cursor.execute("""
        SELECT COALESCE(AVG(milk_yield), 0) as overall_avg
        FROM cows
        WHERE cattle_type IN ('cow', 'buffalo') AND milk_yield IS NOT NULL AND milk_yield > 0
    """)
    ...
    return render_template(
        "admin_milk_production.html",
        milk_data=milk_data_list,
        farmers_list=farmers_list_data,
        total_milk_today=total_milk_today,
        weekly_avg=weekly_avg,
        weekly_total=weekly_total,
        overall_avg=overall_avg
    )
@app.route("/admin/cow_milk_history/<cow_id>")
def cow_milk_history(cow_id):
    if "admin" not in session:
        return redirect(url_for("admin_login"))
    ...
    # Get milk history for last 30 days
    cursor.execute("""
        ...
        LIMIT 30
    """, (cow_id,))
    milk_history = cursor.fetchall()
    
    conn.close()
    
    return render_template("admin_cow_milk_history.html", cow=cow, milk_history=milk_history)
